      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "16px 12px" }}>
          ...
          <div className="hero-content" style={{ position: "relative" }}>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <p style={{ color: "rgba(240,244,255,0.65)", fontSize: 12 }}>
                Gordo · Shun · Dr. Rick · Sexy Tecsy · Lazy Bones
              </p>
            </div>
          </div>
        </div>
        ...
        {liveFixtures.length > 0 && (
          <div className="card" style={{ padding: "10px 14px", marginBottom: 16, display: "flex", gap: 12, overflowX: "auto", alignItems: "center" }}>
            ...
            {liveFixtures.map((f) => (
              <div key={f.fixture.id} className="font-condensed" style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", background: "rgba(239,68,68,0.08)", borderRadius: 6, border: "1px solid rgba(239,68,68,0.2)", flexShrink: 0, fontSize: 13, fontWeight: 600 }}>
        ...
        <div className="card" style={{ overflow: "hidden", marginBottom: 20 }}>
          ...
          {loading ? (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {standings.map((p, i) => (
                  <tr key={p.name} className="standings-row" style={{ borderBottom: i < standings.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                    <td className="rank-cell" style={{ padding: "14px 10px" }}>{RANK_LABELS[i]}</td>
                    <td style={{ padding: "14px 10px" }}>
                      <Link href={`/teams/${p.name.toLowerCase().replace(/\s+/g, "-").replace(/\./g, "")}`} style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 5, height: 24, borderRadius: 3, background: PARTICIPANT_COLORS[p.name] || "#666", flexShrink: 0 }} />
